...
该代码实现了一个 **All-to-All Token Combine（聚合）** 操作，特别针对 **BF16** 数据类型进行了软硬协同优化。其核心作用是将 MoE 模型中分散在各个专家（不同 GPU）上的计算结果加权求和，还原回 Token 的原始布局。

**高性能特征：**
*   **分层通信架构**：先进行节点内（Intra-node via NVLink）归约，再进行节点间（Inter-node via RDMA/IBGDA）归约，显著减少跨节点通信量。
*   **SM 角色分工 (SM Specialization)**：通过 `sm_id` 将 SM 划分为两类（转发者/接收者），避免指令缓存抖动和寄存器溢出。
*   **Hopper 特性拉满**：
    *   **TMA (Tensor Memory Accelerator)**：用于异步数据搬运（Global $\leftrightarrow$ Shared），释放 CUDA Core 用于计算。
    *   **mbarrier**：用于细粒度的异步流水线同步。
    *   **128-bit 向量化**：强制使用 `int4` (128-bit) 进行内存读写。
*   **通信与计算重叠**：使用流水线（Pipeline）掩盖通信延迟。
